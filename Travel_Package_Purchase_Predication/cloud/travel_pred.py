...
model = pickle.load(open('travel_pred.pkl','rb'))   # import module
...
def predict():
    ...
    if prediction == 0:
        return render_template('index.html', prediction_text="Customer Has Purchased The Travel Package").format(prediction)
    else:
        return render_template('index.html', prediction_text="Customer Has Purchased The Travel Package").format(prediction)
